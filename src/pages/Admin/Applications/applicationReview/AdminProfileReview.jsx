...
const DEFAULT_SUMMARY_NARRATIVE =
...
function resolveApplicantCvUrl(app) {
  const stored = app?.onboardingDocuments?.find((d) => d.templateKey === "resume");
  if (stored?.fileData?.trim()) return stored.fileData;
  if (stored?.fileUrl?.trim()) return stored.fileUrl;
  const href = app?.documents?.resume;
  if (href && href !== "#" && String(href).trim()) return href;
  return "/sample.pdf";
}
...
export default function AdminProfileReview({ application, onApproveProfile }) {
  ...
  const cvUrl = resolveApplicantCvUrl(app);
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface-container-lowest p-8 rounded-xl border border-outline-variant/15 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <div className="bg-surface-container-lowest p-8 rounded-xl border border-outline-variant/15 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
              ...
              <p className="text-on-surface-variant leading-relaxed font-light text-lg">{DEFAULT_SUMMARY_NARRATIVE}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <button
              type="button"
              onClick={openCv}
              className="bg-secondary-container rounded-xl p-6 flex flex-col justify-center items-center text-center shadow-lg hover:opacity-95 transition-opacity focus:outline-none focus-visible:ring-2 focus-visible:ring-primary w-full"
            >
