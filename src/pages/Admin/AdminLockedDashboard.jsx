...
import AdminPanelDashboardSkeleton from "./components/AdminPanelDashboardSkeleton";
...
export default function AdminLockedDashboard() {
  ...
  const adminEmail = dashboard?.admin?.email ?? "";
  const adminStatus = dashboard?.admin?.status ?? "";
  const adminRoleLabel = dashboard?.admin?.role ? String(dashboard.admin.role).replace(/_/g, " ") : "";
    ...
    <div className="min-h-screen bg-surface font-body text-on-surface selection:bg-tertiary-fixed-dim selection:text-primary">
      <aside className="fixed left-0 top-0 h-full w-64 border-r border-white/5 bg-[#000615] flex flex-col gap-4 py-8 shadow-[40px_0_40px_rgba(0,6,21,0.04)] z-[60]">
        ...
        <div className="mt-auto px-4">
          <div className="mb-3 rounded-lg border border-white/10 bg-white/5 px-3 py-2 min-h-[3.25rem]">
            {dashboardLoading ? (
              ...
              <>
                ...
                <p className="truncate text-[10px] text-white/60">{adminEmail || "—"}</p>
              </>
            )}
          </div>
          <button
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3 pl-4 border-l border-outline-variant/30">
            <div className="text-right min-w-[7rem]">
              {dashboardLoading ? (
                <div className="animate-pulse space-y-2 flex flex-col items-end" aria-hidden>
                  <div className="h-4 w-28 rounded bg-slate-200" />
                  <div className="h-2.5 w-16 rounded bg-slate-200/80" />
                </div>
              ) : (
                <>
                  <p className="text-sm font-semibold text-[#0B1F3A] font-headline">{adminName || "—"}</p>
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">{adminRoleLabel || "Admin"}</p>
                </>
              )}
            </div>
          </div>
      ...
      <main className="ml-64 pt-24 pb-12 px-12 min-h-screen">
        {dashboardQuery.isError ? (
          <div
            className="rounded-xl border border-error/25 bg-error-container/10 px-6 py-4 text-sm text-on-surface"
            role="alert"
          >
            <p className="font-semibold">Could not load admin dashboard.</p>
            <p className="mt-1 text-on-surface-variant">{dashboardQuery.error?.message || "Please try again."}</p>
            <button
              type="button"
              className="mt-3 rounded-lg bg-primary-container px-4 py-2 text-xs font-bold text-white"
                ...
                <div className="flex items-center gap-8 relative z-10">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-2xl font-bold text-primary font-headline">{adminName || "—"}</h3>
                      <span
                        className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider ${
                          adminStatus === "Active" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"
                        }`}
                      >
                        {adminStatus || "—"}
                      </span>
                    </div>
                    <p className="text-on-surface-variant font-medium text-sm mb-4">{adminEmail || "—"}</p>
                    <div className="flex gap-8">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-primary">{adminRoleLabel || "—"}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right relative z-10">
                  <div className="flex flex-col items-end">
                    ...
                    <p className="text-xs text-on-surface-variant max-w-[180px] leading-relaxed">
                      Employees assigned for timesheet management
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="w-full bg-surface-container-high h-1.5 rounded-full">
                    <div
                      ...
                      style={{ width: `${Math.min(100, (dashboard?.timesheets?.pending ?? 0) * 10)}%` }}
                    />
                  </div>
                  <p className="text-[11px] text-on-surface-variant leading-tight">
                    Pending timesheets are scoped to your assigned employees.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="glass-card rounded-xl shadow-sm overflow-hidden">
                ...
                <tbody className="divide-y divide-outline-variant/10">
                  {employees.map((employee, index) => (
                    <tr
                      className={
                        index % 2 === 1
                          ? "bg-surface-container-low/30 hover:bg-surface-container-lowest/80 transition-colors"
                          : "hover:bg-surface-container-lowest/80 transition-colors"
                      }
                      key={employee.id}
                    >
                      ...
                      <td className="px-8 py-5 text-sm">
                        <span className="bg-sky-50 text-sky-700 px-3 py-1 rounded-full text-[11px] font-bold">
                          {employee.department}
                        </span>
                      </td>
                      <td className="px-8 py-5">
                      ...
                      <td className="px-8 py-5 text-right">
                        <Link
                          className="text-secondary font-bold text-sm hover:underline decoration-2 underline-offset-4"
                          to="/admin-panel/timesheets"
                        >
                          View
                        </Link>
                      </td>
              ...
              <div className="px-8 py-4 bg-surface-container-low/30 border-t border-outline-variant/10 flex justify-between items-center">
                <p className="text-xs text-on-surface-variant">
                  Showing {employees.length} of {dashboard?.assignedEmployees ?? 0} employees assigned
                </p>
              </div>
            </div>
          </>
